...
- The file is `ncm_unblock.ini` beside the bootstrap DLL. It is UTF-8, optionally with a byte-order mark, and holds one `key = value` per line with `#`/`;` comments, blank lines, and case-insensitive keys; there are no sections. The keys are `enabled`, `sidecar_executable`, `http_port`, `https_port`, `automatic_attempts`, and `readiness_timeout_ms`. A relative `sidecar_executable` resolves against the package directory; an absolute one is taken as written. The two ports must both be `auto` or both be fixed and distinct, because a sidecar that redirects HTTPS `CONNECT` needs a distinct pair.
- An absent file means "run with this build's defaults" and the bootstrap continues. A present file that is unreadable, contains an unknown key, a repeated key, a malformed line, or an out-of-range value means the user stated an intent this build cannot honor: the bootstrap reports the offending key and line, keeps forwarding, and installs no routing. Neither outcome may stop a working client, and a partially understood file is never approximated.

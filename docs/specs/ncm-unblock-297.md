...
- The implemented keys `enabled`, `sidecar_executable`, `http_port`, `https_port`, `automatic_attempts`, and `readiness_timeout_ms` describe the retained fallback path and keep their current semantics. Keys for source ordering, quality preference, and cache lifetime are added by the milestone that implements them, not in advance.
...
- **Open.** Extension injection is unproven on this build; the shim's deferral to a live frontend registry is unmeasured; and the client's tolerance for an asynchronously resolved callback is unknown. The last of these is load-bearing: nothing beyond a mocked matcher is built before it is answered.

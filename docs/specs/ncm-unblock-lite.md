...
| Bundled JS privilege inject yields `plLevel=lossless` (and allied levels) under FLAC while URL stays FLAC `br=999000` with `level=lossless` / `encodeType=flac` | Live privilege + player/url probes through proxy 3412 |

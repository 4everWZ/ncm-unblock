...
1. Resolve options: CLI `--ncm` / `--unm` / `--http` / `--https` / optional `--sources`; or exclusive `--args-file <path>` (UTF-8, one argv token per line; empty/`#` lines ignored); or, when argv has no launch tokens, default `launch.args` beside `unm-host.exe`. `--stop` only signals the running host (may combine with `--args-file` on the command line).
...
2. Before start: write UTF-8 `launch.args` next to `unm-host.exe` (one token per line: `--ncm`, path, `--unm`, path, ports, `--sources`, list). Start the host with `betterncm.app.exec` on the quoted host path only (no `lpParameters`, `showWindow=false`) so BetterNCM's ShellExecute path does not strip nested quotes and does not flash a console via `powershell.exe`.
3. Confirm `http://127.0.0.1:<port>/proxy.pac`.
4. Read NCM proxy through `app.getLocalConfig` `["Proxy", ""]`. Write `Type=http`, `Host=127.0.0.1`, `Port=<http>` only when it does not already match. Do not restart NCM when the value is already correct.
5. `onConfig` exposes the four settings. Disable signals `--stop` via `betterncm.app.exec("<host> --stop")` (single safe arg).

V1 does not download UNM, Node, or pick versions.

### Ports and sources

HTTP 3412 / HTTPS 3413 by default. Occupied configured ports fail; the host does not silently pick others. Empty or omitted sources still pass `-o bodian migu kugou` (product default: bodian first under FOLLOW_SOURCE_ORDER so VIP matches do not wait on migu's ~10s miss timeouts; prefers ~320k-capable matchers and UNM's bodian path over broken anonymous kuwo VIP promo clips / qq-without-cookie M500); an explicit Sources list overrides that order. Sources are UNM match-order ids only (for example `bodian,migu,kugou`); host addresses such as `127.0.0.1` are rejected and ignored. The host always overlays `ENABLE_FLAC=true`, `FOLLOW_SOURCE_ORDER=true`, and `ENABLE_LOCAL_VIP=svip` when starting UNM (no plugin toggles), so UNM honors `-o` order instead of racing sources and patches local red+/SVIP membership on vip/info when the NCM session is logged in. The bundled JS fork (when FLAC is on) raises `plLevel`/`dlLevel`/`flLevel` to `lossless` when those keys exist, and sets allied max-level fields (`playMaxLevel`/`downloadMaxLevel`/`maxBrLevel`/`playMaxBrLevel`/`downloadMaxBrLevel`) to `lossless` only on objects that already look like privilege/bitrate payloads (`plLevel` / `playMaxbr` / `downloadMaxbr` / `maxbr` present). It floors `playMaxbr`/`downloadMaxbr`/`maxbr` to `999000` when present and lower, and fills matched `player/url` bodies with `level`/`encodeType` derived from the stream (`flac`/`999000` → `lossless`/`flac`) — official inject only did `none`→`exhigh` / `0`→`320000` and left URL `level`/`encodeType` null. Allied max-level fields must not be written onto every JSON node (that polluted `vip/info` and made the PC username control full-refresh). LOCAL_VIP membership `expireTime` is day-bucketed (`floor(now/864e5)*864e5 + 1y`) rather than upstream’s per-response `now+1y`, so repeated username-menu `vip/info` fetches do not change expire fields within a UTC day (millisecond drift was enough for NCM 2.10.12 to force-refresh the shell after the dropdown opened). The player quality chip can still follow NCM **设置 → 播放 → 在线音质**; UnblockLite does not rewrite that preference. The plugin writes `native/launch.args` and starts `unm-host.exe` with no command-line values (host loads that file); `--stop` is the only process argument used at runtime. Config UI exposes Save & apply and Disable.
...
| Bundled JS privilege inject yields `plLevel=lossless` (and allied levels on privilege objects) under FLAC while URL stays FLAC `br=999000` with `level=lossless` / `encodeType=flac`; non-privilege roots such as anonymous `vip/info` are not stamped with `playMaxLevel*`; authenticated LOCAL_VIP `vip/info` keeps stable membership `expireTime` across repeated probes within a UTC day | Live privilege + player/url + vip/info probes through proxy 3412 |
...
| Plugin start does not invoke `powershell.exe`; host launch args come from `launch.args` beside the host | Source inspection plus live NCM open (no console flash) |

...
| L3i: no-flash host start | Done (ship 0.1.13 live) | NCM open with plugin Enabled does not flash a console; host still reaches Running via PAC 3412 | Root cause: hidden `powershell -EncodedCommand` + `Start-Process` (console subsystem). Fix: plugin writes UTF-8 `native/launch.args`; `betterncm.app.exec` starts quoted `unm-host.exe` with no args (`showWindow=false`); host loads default `launch.args` / optional `--args-file`; `--stop` remains a direct single arg. Installed to `C:\betterncm\plugins\UnblockLite.plugin`; live host cmdline has no args, `native/launch.args` present, PAC 3412 HTTP 200 |

...
| L2: plugin | Done | Disabled/Starting/Running, exec host, PAC check, proxy write-if-needed, `--stop` | `plugin/main.js` + `manifest.json` with `ncm3-compatible`; path search covers runtime + data dir; writes `launch.args` then direct `betterncm.app.exec` (no PowerShell); Sources sanitized to UNM ids; default Sources `bodian,migu,kugou`; Disable button; no image-name `taskkill` |
...
| L3i: no-flash host start | Packaged (ship 0.1.13; live verify pending) | NCM open with plugin Enabled does not flash a console; host still reaches Running via PAC 3412 | Root cause: hidden `powershell -EncodedCommand` + `Start-Process` (console subsystem). Fix: plugin writes UTF-8 `native/launch.args`; `betterncm.app.exec` starts quoted `unm-host.exe` with no args (`showWindow=false`); host loads default `launch.args` / optional `--args-file`; `--stop` remains a direct single arg. Built `out/unblock-lite-0.1.13-x64/UnblockLite.plugin` (focused tests passed) |

## Next action

Reinstall `out/unblock-lite-0.1.13-x64/UnblockLite.plugin`, open NCM, confirm no flash and Running/PAC 3412. Optional deferred: bounded UNM restart; force quality chip when preference is「标准」.

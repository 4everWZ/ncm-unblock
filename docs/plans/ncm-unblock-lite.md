...
| L3: build/package | Done | x64 CMake, focused tests, `UnblockLite.plugin` zip without UNM | `tools/package.ps1` builds `UnblockLite.plugin` (root entries including `certs/`) + outer release ZIP (`0.1.7`) |
...
| L4: lifecycle proof | In progress | Tray hide keeps UNM; tray Exit leaves 0 host/UNM/ports; NCM network works under Running | Host attach+PAC proven; match defaults in 0.1.7 — re-verify live NCM without -202, without Kuwo interstitial, and without ~20s migu-first delay; tray hide/Exit residue still to confirm |

## Next action

Rebuild host + package 0.1.7, then on a real NCM 2.10.12 session: empty Sources → UNM argv has `-o bodian migu kugou` and env has `ENABLE_FLAC=true` + `FOLLOW_SOURCE_ORDER=true`; Save & apply → Running + PAC 200 + no Chromium -202. Spot-check previously interstitial VIP unlocks (real audio in ~1s, not the ~11s Kuwo promo / not migu-timeout lag). Then prove tray hide keeps UNM and tray Exit leaves 0 host/UNM/ports.

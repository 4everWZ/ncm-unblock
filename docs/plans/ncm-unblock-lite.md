...
| L4: lifecycle proof | In progress | Tray hide keeps UNM; tray Exit leaves 0 host/UNM/ports; NCM network works under Running | Host attach+PAC proven; match + VIP + privilege/URL fork + inject gate + stable LOCAL_VIP expire in 0.1.12 — confirm username menu no longer full-refreshes after dropdown; tray hide/Exit residue still to confirm |

## Next action

On a logged-in NCM 2.10.12 session with **0.1.12**: Save & apply / restart so host runs `node …\core\unm-app.js`; confirm username click opens dropdown **without** a subsequent full page refresh, SVIP badge remains, `player/url` still `level=lossless`/`encodeType=flac`, privilege `plLevel=lossless`, and repeated `vip/info` probes keep the same membership `expireTime` within a day. Then prove tray hide keeps UNM and tray Exit leaves 0 host/UNM/ports.

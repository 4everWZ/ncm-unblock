...
| L3e: privilege JS fork | Done (ship 0.1.9) | Bundled patched `core/unm-app.js`; host JS→node launch; plugin prefers JS; package includes NOTICE; live `plLevel=lossless` | Vendored v0.28.0 under `third_party/unm/`; inject floors levels/maxbr under `ENABLE_FLAC`; host `unm mode=js`; official exe still fallback. Player chip may still follow NCM 在线音质 preference |
| L4: lifecycle proof | In progress | Tray hide keeps UNM; tray Exit leaves 0 host/UNM/ports; NCM network works under Running | Host attach+PAC proven; match + local VIP + privilege fork in 0.1.9 — re-verify live NCM UI quality chip after login/restart; tray hide/Exit residue still to confirm |

## Next action

On a logged-in NCM 2.10.12 session with 0.1.9: Save & apply / restart so host runs `node …\core\unm-app.js` with FLAC+FOLLOW+LOCAL_VIP; confirm privilege `plLevel=lossless`, FLAC URL, and whether the quality chip leaves “标准” (check 设置 → 播放 → 在线音质 if not). Then prove tray hide keeps UNM and tray Exit leaves 0 host/UNM/ports.

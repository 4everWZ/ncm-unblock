...
| M0: establish proxy and HTTPS behavior | In progress | Client-local proxy configuration, certificate behavior, traffic ownership, and rollback are reproducible without exposing user data | Synthetic observer test passes; process-local Chromium proxy flag produced no NCM events; UI-based client setting and final recovery remain open |
...
- The controlled command-line experiment is waiting for a normal tray-menu exit. `localdata` bytes remain equal to the protected backup, but its timestamps changed during launch; the private recovery bundle must remain until stopped-process restoration and cleanup are verified.
...
Complete the pending normal NCM exit, restore and verify `localdata` metadata, and remove the private recovery bundle. Then run the redacted HTTP/HTTPS observer matrix through NCM's supported client-local settings UI; do not reuse the falsified command-line flag as an NCM-wide routing mechanism or change system proxy/certificate trust.

...
Exercise M7's failure matrix against the finished launcher path: occupied ports, bad configuration, missing executables, unexpected UNM exit, NCM crash, and launcher termination. Then finish M6's login/search/track-change/playlist/pause-resume matrix and M8's idle, startup, long-run, and native-launcher measurements before preparing the portable M9 artifact.

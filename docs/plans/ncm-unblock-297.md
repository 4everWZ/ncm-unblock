...
| M4: implement the native DLL bootstrap | In progress | The selected proxy preserves the original DLL contract, leaves loader lock before bootstrap, starts the sidecar, installs verified routing, and cleans up with NCM | Application-directory WinMM selection, 193-entry parity, and an absolute-path backend that cannot self-forward are proven against synthetic processes; the loader-lock bootstrap trigger, real-client startup, and routing remain |
...
- Parity is pinned to one host WinMM build (`10.0.26100.8972`). A proxy whose export surface is fixed at build time cannot grow entries a different Windows build may add, so a supported-build rule and a mismatch behavior are unresolved.
- The proxy resolves its backend on the first forwarded call, which can occur while some other module holds the loader lock. That is the ordinary hazard of `LoadLibrary` during initialization and has not been exercised against a real client.
...
Continue the M4 DLL target in this order, and do not run a proxy DLL inside NCM or modify the installed directory until step 1 passes review.

1. Add a loader-lock-safe bootstrap handoff. `DllMain` must not start UNM, wait for readiness, perform network work, or run cleanup coordination synchronously. Decide what the proxy does when the pinned surface and the host WinMM disagree.
2. Run an isolated full-start compatibility experiment against an isolated copy only.
3. Implement the thin bootstrap: load configuration, start the existing sidecar coordinator, establish verified NCM-local routing, and tie cleanup to the exact NCM session.

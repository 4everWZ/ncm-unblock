...
- A `.def` forwarder is closed as a WinMM backend mechanism. A proxy named `winmm.dll` that declares `symbol=WINMM.symbol` emits a name-resolved forwarder that the loader resolves back to the proxy itself. Renaming and redistributing the host system WinMM is separately rejected over module identity, servicing/build coupling, its dependency closure, and redistribution terms. The backend must be reached by absolute system path at runtime.
- A proxy covering only the 48 imported names would link today and still break `GetProcAddress` name lookups, ordinal lookups, and the unnamed ordinal 2. The pinned parity target is 193 ordinal slots, 192 names, and unnamed ordinal 2, currently captured from one host build; portability across supported Windows builds is unresolved.
...
Continue the M4 DLL target in this order, and do not run a proxy DLL inside NCM or modify the installed directory until the first four steps pass review.

1. Generate a repository-owned synthetic x86 fixture pair representing the full 193-entry WinMM export surface, including the unnamed ordinal 2.
2. Establish a backend mechanism that cannot self-forward and does not copy or rename the host system DLL. Validate name identity, ordinal identity, and x86 ABI behavior entirely against synthetic processes first.
...
4. Run an isolated full-start compatibility experiment against an isolated copy only.
5. Implement the thin bootstrap: load configuration, start the existing sidecar coordinator, establish verified NCM-local routing, and tie cleanup to the exact NCM session.

Do not substitute the falsified root flag, private NCM RVAs, fragile coordinate automation, system proxy, or certificate trust for the required DLL routing implementation.

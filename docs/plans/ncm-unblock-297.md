...
- The production session no longer starts the retained sidecar after configuration succeeds. It publishes `injection_pending` and starts no external process; the launcher/sidecar implementation and its focused lifecycle tests remain available behind the fallback boundary.
...
- The M3 application wrapper is implemented without modifying client-owned callback structures. It retains and forwards the original `cef_app_t`, wraps the returned render handler with the same reference ownership as the branch-1916 C API, calls the client's `on_web_kit_initialized` first, and then registers one null-handler marker extension per process. Fresh Win32 fixtures cover every forwarded callback and return value, both wrapper reference lifetimes, exact extension name/source, duplicate-callback suppression, and registration success/failure. This clears the object-wrapping primitive only: interception of the client's `cef_execute_process` import and execution in the exact client remain unverified.
...
1. **M3 — prove injection.** Intercept the client's ordinary `cef_execute_process` import, apply the tested `cef_app_t` wrapper only after the loaded module repeats the pinned API hashes, and observe the null-handler marker in an isolated exact-client renderer. Intercept `cef_initialize` at the same boundary to set `remote_debugging_port` in a size-checked settings copy, because that turns every later gate into something observable rather than inferred.

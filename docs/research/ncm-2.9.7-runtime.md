...
The production bootstrap now waits outside loader lock for the loaded `cloudmusic.dll` and `libcef.dll`, repeats both pinned API hashes, resolves `cef_execute_process` and `cef_register_extension`, and replaces only the named ordinary `cef_execute_process` IAT slot. Its wrapper preserves the client's application and render-handler callbacks and registers a native-handler M3 extension after the client's own `on_web_kit_initialized` callback. Ownership follows CEF 1916 `CefV8HandlerCToCpp::Wrap`: the product allocates the handler at ref=1, adds one transfer reference, calls `cef_register_extension`, and never releases afterward. Wrap always consumes the transfer with one `UnderlyingRelease`; success retains the remaining reference for process lifetime, while failure destroys through the temporary RefPtr. Synthetic PE32 fixtures mimic that Wrap release-then-retain/destroy contract and prove successful replacement and hash-mismatch refusal before this path is allowed near the target.

A fresh isolated Release run against the signed `2.9.7.199711` executable with the native-handler build produced a main window and four live client processes. Three fixed-classification reports recorded `hook=installed`; one renderer recorded `registration=succeeded marker=2`, proving that the client called the wrapped render callback, CEF accepted the extension with Wrap-style ownership transfer, and a live V8 context executed `ncmUnblock297Marker`. The root and another process remained `registration=not_attempted marker=0`, as expected for roles that did not reach the renderer callback during the observation window. The run reclaimed only the four processes whose executable paths were under the private experiment root, verified the real `localdata` fingerprint unchanged, and removed the experiment directory.

That result clears M3 observation: registration alone is not the gate, and the native Execute counter published as `marker=N` reached `marker>0` with a normal window. A parallel Debug proxy run AV'd early (`0xC0000005`) before registration; Release remains the exact-client validation configuration. Remote-debugging stays deferred and is not required for this gate.

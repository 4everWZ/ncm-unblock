...
V1 does not download UNM. Leave **Sources** empty for the product default match order `bodian,migu,kugou` (~320k-oriented; FLAC attempted when a source supports it; `bodian` first so FOLLOW_SOURCE_ORDER does not wait on migu timeouts; `kuwo` is omitted because VIP tracks can resolve to a promo clip). Override with other UNM match-order ids only when you need a different order — never put `127.0.0.1` there (that is the proxy host, set automatically).
